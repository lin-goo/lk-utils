...
from .parser import parse_make_and_model
    ...
    def merge_marker_data(data):
        ...
        if isinstance(data.get("marker-colors"), List):
            for k, v in enumerate(data["marker-colors"]):
                if k < mlen:
                    marker_colors[k] = [f"#{color}" for color in v.split("#") if color]
        else:
            marker_colors[0] = [
                f"#{color}" for color in data["marker-colors"].split("#") if color
            ]
